...
`invoices` stays keyed to `orders.id` (the per-shop sub-order, not `order_groups`) because each shop needs its own GST invoice under its own GSTIN — this was already correct in v1 and needed no change. `wishlists`, `product_cross_sell`, `recurring_lists`/`recurring_list_items` (organizer, reminder-mode per your confirmed decision) all carry over unchanged.
...
**RLS is enabled on every table with no exceptions** (already stated throughout §4). On top of that: **any write that involves cross-table business logic — checkout, order-status transitions, rider assignment, invoice generation — goes through a `SECURITY DEFINER` Postgres function (an "RPC"), never a raw client-side `INSERT`/`UPDATE` against multiple tables.** The client (Flutter app or Next.js website) calls either Supabase's `rpc()` client method directly for simple cases, or an Edge Function route for anything that also needs Zod validation or a third-party call (Razorpay/PayU, FCM) in the same operation — matching the Hono-route-calling-into-`db` shape Baker Ally already uses. Simple single-table reads/writes that RLS alone can safely gate (read your own addresses, add a wishlist item) don't need an RPC wrapper — reserve RPCs for the operations where correctness depends on more than one row being consistent.
...
### Sprint 12 (2 weeks) — Shopkeeper analytics, ledger, admin completion
...
- **Exit criteria:** a shopkeeper can see their own real revenue numbers and outstanding ledger balance; admin can change the platform delivery fee and see it reflected on the next new order.
...
- Actual Razorpay/PayU commercial terms (needed before Sprint 8 starts, not before).
