...
| File | Purpose |
|---|---|
| [proximity_backend/deno.json](../../proximity_backend/deno.json) | **Real fix, found during this sprint's second verification pass, not new feature work:** every import-map key rewritten from a bare specifier (`"zod"`) to the literal `"npm:zod"` form every file in this backend actually imports by -- the bare-key version has been silently unpinned since Sprint 1 (see "Second verification pass" below). `deno.lock` regenerated by Deno itself against the corrected map. |
...
## Second verification pass (prompted by "is Sprint 6 actually complete?") -- two more real issues caught

Same discipline Sprint 3's own second pass applied: re-reading every new file with fresh eyes and re-running the actual tooling, rather than trusting this sprint's own first self-report. Found two real issues, one of them project-wide (not Sprint-6-specific) and significant enough to fix immediately rather than defer.

1. **`deno.json`'s entire `imports` map has been silently inert since Sprint 1 -- every backend dependency has actually been running unpinned this whole project, not on the versions every sprint's write-up claimed.** `deno.json`'s import-map keys are bare specifiers (`"zod": "npm:zod@^3.23.0"`, `"drizzle-orm": "npm:drizzle-orm@^0.36.0"`, etc.), but **every single file in this backend imports via the literal `"npm:zod"`/`"npm:drizzle-orm"`/`"npm:hono"` specifier form** (`import { z } from "npm:zod";`) -- Deno import maps only remap a specifier that matches the map key *exactly as written in the import statement*, and `"npm:zod"` never matches a key of `"zod"`. Confirmed, not assumed: `deno info` on the real entrypoint showed the actually-resolved versions as `zod@4.4.3` (not `^3.23.0`), `@hono/zod-validator@0.8.0` (not `^0.4.1`), `drizzle-orm@0.45.2` (not `^0.36.0`) -- three major-version jumps past what every prior sprint's write-up believed was pinned, silently picked up by Deno's unconstrained `npm:package@*` resolution. **The good news, also verified rather than assumed:** `deno check` passes clean against both the drifted versions and the actually-intended pinned ones (tested by rewriting the import map keys to the literal `"npm:zod"` form Deno actually matches against, in a throwaway copy first, confirming the resolution genuinely changes before touching the real files) -- so nothing in this codebase has silently depended on a zod-4-only or drizzle-0.45-only API. Fixed for real: `deno.json`'s import keys now read `"npm:zod"`/`"npm:drizzle-orm"`/`"npm:hono"`/etc. (matching what the code actually writes), `deno.lock` regenerated by Deno itself (not hand-edited) against the corrected map, `deno check` re-verified clean. This was never a Sprint 6 bug specifically -- it's been true since Sprint 1's `deno.json` was first written -- but it directly undermines every prior sprint's "verified: deno check passes" claim in the sense that "passes" was never proven against the versions those write-ups named, so it's recorded here rather than silently patched without comment.
2. **Two real Flutter list-reconciliation bugs -- `CartItemTile`/`ShopCartSection` built without `Key`s in lists that shrink.** Both are `StatefulWidget`s holding real per-row state (`CartItemTile`'s in-flight `_busy` flag, `ShopCartSection`'s collapsed/expanded flag) rendered via `for (final x in list) Widget(...)` with no `key:` -- textbook Flutter footgun: without an explicit key, the framework reconciles list children by *position*, not identity, so removing an item from the middle of the list (any single-item removal, or "Remove all from this shop" collapsing the shop list) can hand a stale `_busy`/`_expanded` value to whatever row shifts into the removed one's old slot. Fixed with `ValueKey(item.id)` on `CartItemTile` and `ValueKey(entry.key)` (the shop id) on `ShopCartSection`. `RecommendedProductCard`/`ShopCard` were checked too and don't need this -- both are stateless, so there's no per-row state to bleed.

Re-verified after both fixes: `deno check supabase/functions/api/index.ts` clean, `flutter analyze` still 0 errors/0 warnings (same 17 pre-existing style infos).

Also verified, not just re-inspected: `rpc_add_to_cart`'s get-or-create-cart + upsert-item logic (traced by hand against Postgres's actual `INSERT ... ON CONFLICT ... RETURNING` semantics -- both paths return exactly one row either way), `cart_items`' RLS scoping (a backstop only per §5.1, but the subquery shape is correct), the `inArray`/empty-array handling every new route relies on (checked the *actually-resolved* `drizzle-orm` source directly rather than assumed -- confirmed `inArray(col, [])` emits `sql\`false\``, not broken SQL, so `recommendations.ts`'s image-fetch step never breaks even when every candidate gets filtered out), and the `recommendations.ts` `GROUP BY`/`HAVING` shape (every non-aggregated `SELECT`/`HAVING` column traced against the `GROUP BY` list by hand). No further issues found in these.
