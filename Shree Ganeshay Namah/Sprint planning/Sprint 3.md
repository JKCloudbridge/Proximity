...
A second, independent verification pass (re-running `deno check`/`flutter analyze`/`npm run build|lint|audit` and re-reading every new migration and route file with fresh eyes, prompted by the user explicitly asking "is Sprint 3 complete") caught two more real issues after the sprint was first called done — recorded here rather than quietly folded into the list above, since "verified" should mean something:
...
`deno lint` (as opposed to `deno check`, the gate this project actually uses) also flags every `npm:`-prefixed import across the *entire* backend — including Sprint 1/2's `shops.ts`/`admin.ts` — for not pinning a version inline, even though `deno.json` does pin versions via its import map that these files don't route through. Checked this is pre-existing and codebase-wide, not a Sprint 3 regression, and not something this sprint's write-up should claim credit or blame for either way.
